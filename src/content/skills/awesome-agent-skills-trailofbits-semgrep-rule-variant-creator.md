---
title: "Semgrep RULE Variant Creator 測試 Skill"
description: "Port existing Semgrep rules to new target languages 搭配 test-driven validation。"
category: "testing"
tags:
...
license: "MIT"
originalName: "trailofbits/semgrep-rule-variant-creator"
originalDescription: "Port existing Semgrep rules to new target languages with test-driven validation"
activation: "當你需要Port existing Semgrep rules to new target languages 搭配 test-driven validation時使用。"
useCases:
  - "需要Port existing Semgrep rules to new target languages 搭配 test-driven validation。"
  - "想使用 trailofbits/semgrep-rule-variant-creator 這類已整理好的 Agent Skill，而不是從零撰寫 prompt。"
...
Port existing Semgrep rules to new target languages 搭配 test-driven validation。這筆資料來自 `awesome-agent-skills` 的索引清單，原始專案是 `trailofbits/semgrep-rule-variant-creator`。
