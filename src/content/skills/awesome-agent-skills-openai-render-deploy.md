---
title: "Render Deploy 雲端部署 Skill"
description: "部署 applications to Render's cloud platform 使用 Git-backed services。"
category: "cloud"
tags:
...
license: "MIT"
originalName: "openai/render-deploy"
originalDescription: "Deploy applications to Render's cloud platform using Git-backed services"
activation: "當你需要部署 applications to Render's cloud platform 使用 Git-backed services時使用。"
useCases:
  - "需要部署 applications to Render's cloud platform 使用 Git-backed services。"
  - "想使用 openai/render-deploy 這類已整理好的 Agent Skill，而不是從零撰寫 prompt。"
...
部署 applications to Render's cloud platform 使用 Git-backed services。這筆資料來自 `awesome-agent-skills` 的索引清單，原始專案是 `openai/render-deploy`。
