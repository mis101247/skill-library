---
title: "Figma Skill"
description: "Use the Figma MCP server to fetch design context 與 translate nodes into production code。"
category: "design"
tags:
...
license: "MIT"
originalName: "openai/figma"
originalDescription: "Use the Figma MCP server to fetch design context and translate nodes into production code"
activation: "當你需要Use the Figma MCP server to fetch design context 與 translate nodes into production code時使用。"
useCases:
  - "需要Use the Figma MCP server to fetch design context 與 translate nodes into production code。"
  - "想使用 openai/figma 這類已整理好的 Agent Skill，而不是從零撰寫 prompt。"
...
Use the Figma MCP server to fetch design context 與 translate nodes into production code。這筆資料來自 `awesome-agent-skills` 的索引清單，原始專案是 `openai/figma`。
