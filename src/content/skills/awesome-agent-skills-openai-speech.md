---
title: "Speech 設計 Skill"
description: "產生 spoken audio from text 使用 OpenAI's API 搭配 built-in voices。"
category: "design"
tags:
...
license: "MIT"
originalName: "openai/speech"
originalDescription: "Generate spoken audio from text using OpenAI's API with built-in voices"
activation: "當你需要產生 spoken audio from text 使用 OpenAI's API 搭配 built-in voices時使用。"
useCases:
  - "需要產生 spoken audio from text 使用 OpenAI's API 搭配 built-in voices。"
  - "想使用 openai/speech 這類已整理好的 Agent Skill，而不是從零撰寫 prompt。"
...
產生 spoken audio from text 使用 OpenAI's API 搭配 built-in voices。這筆資料來自 `awesome-agent-skills` 的索引清單，原始專案是 `openai/speech`。
