---
title: "Open Code Review 程式碼審查 Skill"
description: "使用 alibaba/open-code-review 的 `ocr` CLI 對 Git 變更執行 AI code review，產生可定位到行的審查意見。"
category: "review"
tags:
  - open-code-review
  - code review
...
sourcePath: "skills/open-code-review/SKILL.md"
sourceUrl: "https://github.com/alibaba/open-code-review/blob/main/skills/open-code-review/SKILL.md"
localPath: "software-engineering-prompt-repos/open-code-review/skills/open-code-review/SKILL.md"
license: "Apache-2.0"
originalName: "open-code-review"
originalDescription: "Performs AI-powered code review on Git changes using the `ocr` CLI from alibaba/open-code-review. Use when the user asks to review code, review a pull request, review staged/unstaged changes, review a commit, or compare branches for code quality issues."
activation: "當使用者要 review working tree、PR、指定 commit 或 branch diff，並希望取得 line-level AI code review 意見時啟用。"
useCases:
  - "review staged、unstaged 與 untracked changes。"
  - "review 指定 commit 或比較兩個 branch。"
  - "用專案背景與自訂規則提高 code review 命中率。"
workflow:
  - "先確認 `ocr` CLI 已安裝，並用 `ocr llm test` 檢查 LLM 連線。"
  - "分析 review 目標，整理 business context，透過 `--background` 傳給 OCR。"
  - "用 `ocr review --audience agent` 執行 review，依需求加入 commit、from/to、preview 等參數。"
  - "把 review output 分成 High、Medium、Low；回報時只保留有價值的 High / Medium。"
  - "若使用者明確要求修正，才套用安全且明確的建議，並再次驗證。"
files:
  - "SKILL.md"
cautions:
  - "LLM token 或 config 不可硬編碼，缺憑證時要請使用者設定。"
  - "一律使用 `--audience agent`，避免 progress UI 汙染輸出。"
  - "只有使用者明確要求 review and fix 時，才自動修改程式碼。"
skillBody: |
  ---
...
這是目前 repo 裡最完整的工程型 Skill：它把 code review 的前置檢查、CLI 執行、背景脈絡、輸出分級、修正許可權與驗證方式全部寫清楚。

## 為什麼值得收錄

它非常符合 Skill 的定義：不只是「請幫我 review code」這種 prompt，而是能讓 agent 依照固定流程操作實際工具、處理錯誤、分類 findings，並在必要時回到程式碼修正。
