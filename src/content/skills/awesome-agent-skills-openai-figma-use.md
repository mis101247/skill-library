...
description: "Prerequisite skill 用於 every use_figma tool call — write/read actions in Figma context。"
category: "design"
tags:
...
license: "MIT"
originalName: "openai/figma-use"
originalDescription: "Prerequisite skill for every use_figma tool call — write/read actions in Figma context"
activation: "當你需要Prerequisite skill 用於 every use_figma tool call — write/read actions in Figma context時使用。"
useCases:
  - "需要Prerequisite skill 用於 every use_figma tool call — write/read actions in Figma context。"
  - "想使用 openai/figma-use 這類已整理好的 Agent Skill，而不是從零撰寫 prompt。"
...
Prerequisite skill 用於 every use_figma tool call — write/read actions in Figma context。這筆資料來自 `awesome-agent-skills` 的索引清單，原始專案是 `openai/figma-use`。
