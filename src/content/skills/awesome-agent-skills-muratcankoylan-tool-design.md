...
description: "建置 tools that agents can use effectively, including architectural reduction 模式。"
category: "design"
tags:
...
license: "MIT"
originalName: "muratcankoylan/tool-design"
originalDescription: "Build tools that agents can use effectively, including architectural reduction patterns"
activation: "當你需要建置 tools that agents can use effectively, including architectural reduction 模式時使用。"
useCases:
  - "需要建置 tools that agents can use effectively, including architectural reduction 模式。"
  - "想使用 muratcankoylan/tool-design 這類已整理好的 Agent Skill，而不是從零撰寫 prompt。"
...
建置 tools that agents can use effectively, including architectural reduction 模式。這筆資料來自 `awesome-agent-skills` 的索引清單，原始專案是 `muratcankoylan/tool-design`。
